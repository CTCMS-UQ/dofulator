...
inline static void vec_sub(const double NOALIAS_ARR(l, 3), const double NOALIAS_ARR(r, 3), double NOALIAS_ARR(out, 3)) {
  out[0] = l[0] - r[0];
  out[1] = l[1] - r[1];
  out[2] = l[2] - r[2];
}

static inline void vec_cross(const double NOALIAS_ARR(v1, 3), const double NOALIAS_ARR(v2, 3), double NOALIAS_ARR(c, 3)) {
